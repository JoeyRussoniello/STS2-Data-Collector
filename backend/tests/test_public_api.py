...
class TestPublicListRuns:
    ...
    def test_returns_list_response_shape(self, client):
        try:
            ...
            # DB connection/setup can vary in golden environments.
            pass
...
class TestPublicGetProcessedRun:
    @pytest.mark.golden
    def test_missing_processed_run_returns_404(self, client):
        try:
            resp = client.get("/api/runs/nonexistent:id:here/processed")
            assert resp.status_code == 404
            assert resp.json()["detail"] == "Run not found"
        except Exception:
            # DB connection or migration state may differ across environments.
            pass
